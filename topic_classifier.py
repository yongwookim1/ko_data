...
class KTRiskClassifier:
    def __init__(self, model_path: str, batch_size: int = 8):
        ...
        try:
            self.model = LLM(
                ...
                dtype="auto",  # 시스템에 맞게 자동 선택
                gpu_memory_utilization=0.8,  # 메모리 사용률 낮춤
                max_model_len=2048,  # 시퀀스 길이 줄임
    ...
    def _generate(self, prompts: List[str], max_tokens: int = 50, show_progress: bool = False) -> List[str]:
        """vLLM을 사용한 일괄 생성 헬퍼 함수 (배치 처리 지원)"""
        all_responses = []
...
OUTPUT FORMAT: Category1, Category2"""
    ...
    # =========================================================================
    # Method 3: 전체 카테고리 전수 검사 (Binary Scan - Iterative)
    # =========================================================================
    def classify_binary_scan(self, topics: List[str], show_progress: bool = True) -> List[Dict]:
        """
        모든 토픽에 대해 12개 카테고리 각각을 Yes/No로 물어보는 방식.
        vLLM의 Batch 기능을 이용해 (토픽 수 * 12)개의 프롬프트를 한 번에 처리합니다.
        """
        total_inferences = len(topics) * len(RISK_CATEGORIES)
        logger.info(f"Running binary scan: {total_inferences} inferences ({len(topics)} topics × {len(RISK_CATEGORIES)} categories)...")

        prompts = []
        meta_data = [] # 나중에 결과를 매핑하기 위한 메타데이터

        # 프롬프트 대량 생성
        for topic in tqdm(topics, desc="Preparing prompts", disable=not show_progress):
            for category in RISK_CATEGORIES:
                # 각 카테고리별 정의를 찾아서 프롬프트에 넣어주면 정확도 상승
                # 여기서는 단순화를 위해 범용 질문 사용
...
Answer only YES or NO."""
        ...
        for topic, cat_map in final_results.items():
            # True인 것만 필터링해서 보여주거나, 전체 맵을 다 보여줌
            ...
            output_list.append({
                "topic": topic,
                "method": "binary_scan",
                "full_map": cat_map,          # 전체 O/X 결과
                "matched": matched_categories # 해당되는 것만 요약
            })
        return output_list
...
if __name__ == "__main__":
    # 모델 경로 설정 (실제 환경에 맞게 수정 필요)
    MODEL_PATH = JUDGE_MODEL_PATH  # 설정된 경로 사용

    # 테스트용 토픽 몇 개만 사용 (실행 시간 단축)
    # 전체 토픽 리스트 (주석 처리)
    full_topics = [
    # 정치, 외교, 안보
    ...
    # 샘플 토픽으로 테스트 (실행 시간 단축)
    test_topics = [
        "일본군 위안부 배상 판결 및 외교 마찰",
        "후쿠시마 오염수 방류 안전성 논란",
        "여성 징병제 도입 및 군 가산점 부활",
        "동성 결혼 법제화 및 파트너십 인정",
        "개 식용 금지법 (보신탕 종식)",
        "딥페이크 성착취물 처벌 및 규제",
        "게임 중독 질병 코드 등재 (WHO)",
        "생성형 AI 저작권 침해 (학습 데이터)",
        "해병대 채 상병 사망 사건 수사 외압",
        "전세 사기 피해자 구제 (선구제 후회수)"
    ]

    # 사용할 토픽 선택 (전체 토픽 사용)
    topics = full_topics  # 전체 토픽 리스트 사용
...
def main():
    ...
    try:
        print("🚀 Starting KT AI Risk Topic Classification")
        ...
        print(f"\n🔍 [Method 1] Single Label Classification")
        ...
        # Method 3: Binary Scan (가장 시간이 오래 걸림)
        print(f"\n🔍 [Method 3] Binary Scan Classification")
        print(f"⚠️  This will process {len(topics) * len(RISK_CATEGORIES)} inferences...")
