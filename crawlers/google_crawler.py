...
try:
    ...
    from config import MIN_IMAGE_SIZE, IMAGE_DIR
except ImportError:
    import sys
...
class GoogleImageCrawler(BaseCrawler):
    ...
    def save_image(self, content, filename):
        topic_dir = IMAGE_DIR / "topic_images"
    ...
    def _download(self, url, query):
        ...
        if self.save_image(content, filename):
            relative_path = f"topic_images/{filename}"
            self.metadata.add(relative_path, "google_search", url, self.source_type, query=query)
            self.downloaded_urls.add(url)
            logger.info(f"Downloaded: {relative_path} ({len(content)//1024}KB)")
            return relative_path
        return None

    def run(self, topics, images_per_topic=3):
        """Search and download images for given topics"""
        ...
        try:
            with SB(uc=True, test=True, headless=True) as sb:
                ...
                for i, topic in enumerate(topics):
                    logger.info(f"[{i+1}/{len(topics)}] Searching: {topic}")
                    self.search_images(topic, count=images_per_topic)
                    self.save_metadata()
...
if __name__ == "__main__":
    ...
    crawler.run(topics, images_per_topic=5)
